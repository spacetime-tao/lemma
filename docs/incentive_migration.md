...
|-----------|----------------|
| Proof-only target | Rewards are binary Lean pass/fail for published theorem statements. See [proof-only-incentives.md](proof-only-incentives.md). |
...
| Proof-only live score | A Lean-verified proof enters scoring as a binary pass. |
...
| Training export profiles | **`LEMMA_TRAINING_EXPORT_JSONL`** optional JSONL; **`LEMMA_TRAINING_EXPORT_PROFILE`** = **`full`** (proof + optional labels + optional `proof_metrics` + `pareto_weight`) or **`summary`** (schema v2 — identifiers/provenance without proof, metrics, labels, or weights). See [training_export.md](training_export.md). |
...
- Judge profile attest: [judge-profile-attest.md](judge-profile-attest.md), `lemma/validator/judge_profile_attest.py`, `lemma/validator/service.py`, `lemma/cli/validator_check.py`
