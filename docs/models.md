...
One pinned validator scoring profile per subnet: `uv run lemma meta` → `judge_profile_sha256` → optional `JUDGE_PROFILE_SHA256_EXPECTED`. The hash name is compatibility baggage; the profile should cover deterministic scoring/cadence/verification knobs that affect weights.

After changing scoring knobs, problem cadence, or verifier policy, rerun `uv run lemma meta` and redistribute hashes.

## Miners (prover)

Use a model that writes valid `Submission.lean` reliably. Recommended baseline on Chutes: a strong reasoning model such as `deepseek-ai/DeepSeek-V3.2-TEE`, or another prover model you operate reliably.
