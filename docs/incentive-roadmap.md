...
Track in issues or refactors as capacity allows: remaining human-friendly CLI wrappers and other small runtime glue. Recent cleanup removed `validator/query.py`, split catalog dev helpers out of runtime, merged validator startup gates shared by `validator-check` and `service`, collapsed warm Lake-cache behavior to the single `LEMMA_LEAN_ALWAYS_CACHE_GET` override, inlined trace-length scoring, and removed single-use miner/validator helpers.
