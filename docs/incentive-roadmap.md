...
- [x] **Training export** — Documented gaming/leakage, proof-metrics calibration, and sybil/Pareto replay collection ([training_export.md](training_export.md)); **`LEMMA_TRAINING_EXPORT_PROFILE=reasoning_only`** omits proof, proof metrics, judge rubric, and Pareto weights (`lemma/validator/training_export.py`).
