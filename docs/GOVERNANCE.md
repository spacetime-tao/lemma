...
Document **`DENDRITE_TIMEOUT_S`**, **`LEAN_VERIFY_TIMEOUT_S`**, **`LEMMA_PROBLEM_SEED_MODE`** (same **`subnet_epoch`** vs **`quantize`** across validators), **`LEMMA_PROBLEM_SEED_QUANTIZE_BLOCKS`** when using quantize / fallback, **`EMPTY_EPOCH_WEIGHTS_POLICY`**, **`LEAN_SANDBOX_*`**, **`JUDGE_*`**, **`OPENAI_*`**, **`LEMMA_COMPARATOR_*`**. **`DENDRITE_TIMEOUT_S`** should match across validators (not enforced on-chain).
