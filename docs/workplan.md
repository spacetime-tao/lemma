...
1. Replace or reduce `proof_intrinsic_score`; length is a weak proxy for mathematical value. **Default weight lowered to `0.10`; decision note added:** [proof-intrinsic-decision.md](proof-intrinsic-decision.md). Next behavior change should prototype a Lean-backed replacement, not add more regex padding checks.
...
1. Miner verify attest: keep full validator Lean verify as default; do not let attest-only paths inflate credibility. **Done for credibility; verify batch now isolates per-UID verifier exceptions.**
...
6. Simplify small glue APIs when an audit item has a single unused path. **Started: removed the unused third return from `apply_ema_to_entries`; consolidated dedup best-by-key logic; removed unused scoring package re-exports; removed duplicate `ScoredEntry.composite`.**
