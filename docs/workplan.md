...
6. Simplify small glue APIs when an audit item has a single unused path. **Ongoing:** removed the unused third return from `apply_ema_to_entries`; consolidated dedup best-by-key logic; removed unused scoring package re-exports; removed duplicate `ScoredEntry.composite`; inlined trace-length scoring; inlined miner default priority; removed the single-use validator UID helper; added direct reward assembly coverage.
