...
def test_replay_compares_current_partition_and_clone_pressure(tmp_path) -> None:
    ...
    base = replay_epoch(report.replay_rows, name="base", identical_dedup=False, coldkey_partition=True)
    assert base.identical_dropped == 0
    assert 3 in base.weights
    ...
    assert legacy.identical_dropped == 1
    assert 3 not in legacy.weights
    ...
    assert exact.extra_share > 0.0
    assert rewritten.extra_share == pytest.approx(exact.extra_share)
    ...
    assert "summary_exact_clone_extra_share: n=1 max=0.2667 mean=0.2667" in rendered
    assert "summary_rewritten_clone_extra_share: n=1" in rendered
    assert "summary_rewritten_clone_group_share: n=1" in rendered
    assert "base: weighted_uids=3" in rendered
    assert "legacy_identical_dedup: weighted_uids=2" in rendered
...
def test_replay_uses_coldkey_rows_when_present(tmp_path) -> None:
    ...
    _write_jsonl(
        ...
        [
            ...
            _row(uid=3, composite=0.7, proof="theorem v : True := by trivial\n", trace="other", coldkey="other"),
        ],
    )
    ...
    no_coldkey = replay_epoch(rows, name="no_coldkey", identical_dedup=False, coldkey_partition=False)

    assert base.coldkey_partitioned == 2
    assert base.weights == {1: 0.25, 2: 0.25, 3: 0.5}
    assert no_coldkey.coldkey_partitioned == 0
    assert no_coldkey.weights == {1: 1 / 3, 2: 1 / 3, 3: 1 / 3}
...
def test_replay_fingerprint_uses_theorem_statement_when_present(tmp_path) -> None:
    ...
    base = replay_epoch(rows, name="base", identical_dedup=True, coldkey_partition=True)
